...
import { UserImport, ExportManifest } from "../types/user-export";
import { parseCsv, prepareDocForImport } from "./csvParsers";
 ...
 */
export const processUserImportOnUpdate = onDocumentUpdated(
  {
  ...
  async (event) => {
    ...
    const before = event.data.before.data() as Omit<UserImport, "id">;
    const after = event.data.after.data() as Omit<UserImport, "id">;
...
async function processImport(
  ...
  try {
    ...
    // Import data
    const importedCounts = await importData_(
      db,
      storage,
      userId,
      zipData,
...
interface ZipData {
  ...
  transactionCsvs: Map<string, string>;
  filesCsv: string | null;
  fileConnectionsCsv: string | null;
  partnersCsv: string | null;
  categoriesCsv: string | null;
  noReceiptCategoriesCsv: string | null;
}

/**
 * Extract data from ZIP file
 */
async function extractZipData(zipContent: Buffer): Promise<ZipData> {
  const data: ZipData = {
    manifest: null,
    userData: null,
    sourcesCsv: null,
  ...
  return new Promise((resolve, reject) => {
    const readable = Readable.from(zipContent);
    ...
    readable
      ...
      .on("entry", (entry) => {
        ...
        const extractPromise = new Promise<void>((resolveEntry) => {
          entry
...
async function importData_(
  ...
  if (zipData.sourcesCsv) {
    ...
    const sources = parseCsv(zipData.sourcesCsv);
  ...
  if (zipData.filesCsv) {
    ...
    const files = parseCsv(zipData.filesCsv);
  ...
  if (zipData.fileConnectionsCsv) {
    ...
    const connections = parseCsv(zipData.fileConnectionsCsv);
  ...
  if (zipData.partnersCsv) {
    ...
    const partners = parseCsv(zipData.partnersCsv);
  ...
  if (zipData.categoriesCsv) {
    ...
    const categories = parseCsv(zipData.categoriesCsv);
  ...
  if (zipData.noReceiptCategoriesCsv) {
    ...
    const noReceiptCategories = parseCsv(zipData.noReceiptCategoriesCsv);
    counts.noReceiptCategories = await importDocuments(
      db,
      "noReceiptCategories",
      noReceiptCategories,
      userId
  ...
  if (zipData.manifest?.includesStorageFiles) {
    ...
    const storageCount = await importStorageFiles(
      storage,
      userId,
      zipContent,
      importRef
...
async function importDocuments(
  ...
  for (let i = 0; i < documents.length; i += BATCH_SIZE) {
    ...
    for (const doc of chunk) {
      ...
      const prepared = prepareDocForImport(doc, userId);
