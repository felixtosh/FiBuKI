...
exports.processUserExportScheduled = exports.processUserExportOnCreate = void 0;
 ...
 */
exports.processUserExportScheduled = (0, scheduler_1.onSchedule)({
    ...
    const stalledThreshold = firestore_2.Timestamp.fromDate(new Date(Date.now() - 10 * 60 * 1000) // 10 minutes ago
    );
    ...
    for (const doc of pendingExports.docs) {
        ...
        if (exportData.status === "processing" &&
            ...
            exportData.startedAt.toMillis() > stalledThreshold.toMillis()) {
            continue;
        }
...
async function processExport(exportId, exportData) {
    ...
    try {
        ...
        const zipBuffer = await createExportZip(data, exportId, userId, includeStorageFiles, storage, exportRef, startTime);
        // Check timeout
        if (Date.now() - startTime > PROCESSING_TIMEOUT_MS) {
        ...
        await db.collection(`users/${userId}/notifications`).add({
            ...
            message: `Your data export is ready for download. It will expire in ${user_export_1.EXPORT_EXPIRY_DAYS} days.`,
 ...
 */
async function collectUserData(db, userId, exportRef, startTime) {
    const data = {
        sources: [],
    ...
    for (const source of data.sources) {
        ...
        const transactionsSnap = await db
            ...
            .get();
        data.transactionsBySource[source.id] = transactionsSnap.docs.map((doc) => ({
            id: doc.id,
            ...doc.data(),
        }));
 ...
 */
async function createExportZip(data, exportId, userId, includeStorageFiles, storage, exportRef, startTime) {
    return new Promise((resolve, reject) => {
        ...
        // Add manifest
        const manifest = {
            version: user_export_1.EXPORT_FORMAT_VERSION,
        ...
        // Add sources CSV
        const sourcesCsv = (0, csvGenerators_1.generateCsv)(data.sources, csvGenerators_1.sourcesColumns);
        archive.append(sourcesCsv, { name: `${folderName}/sources.csv` });
        // Add transactions CSVs (one per source)
        for (const source of data.sources) {
            const sourceId = source.id;
            const sourceName = (source.name || "unnamed").replace(/[^a-zA-Z0-9-_]/g, "_");
            const transactions = data.transactionsBySource[sourceId] || [];
            if (transactions.length > 0) {
                const transactionsCsv = (0, csvGenerators_1.generateCsv)(transactions, csvGenerators_1.transactionsColumns);
        ...
        // Add files CSV
        const filesCsv = (0, csvGenerators_1.generateCsv)(data.files, csvGenerators_1.filesColumns);
        archive.append(filesCsv, { name: `${folderName}/files.csv` });
        // Add file connections CSV
        const fileConnectionsCsv = (0, csvGenerators_1.generateCsv)(data.fileConnections, csvGenerators_1.fileConnectionsColumns);
        archive.append(fileConnectionsCsv, { name: `${folderName}/fileConnections.csv` });
        // Add partners CSV
        const partnersCsv = (0, csvGenerators_1.generateCsv)(data.partners, csvGenerators_1.partnersColumns);
        archive.append(partnersCsv, { name: `${folderName}/partners.csv` });
        // Add categories CSV
        const categoriesCsv = (0, csvGenerators_1.generateCsv)(data.categories, csvGenerators_1.categoriesColumns);
        archive.append(categoriesCsv, { name: `${folderName}/categories.csv` });
        // Add no-receipt categories CSV
        const noReceiptCategoriesCsv = (0, csvGenerators_1.generateCsv)(data.noReceiptCategories, csvGenerators_1.noReceiptCategoriesColumns);
        ...
        if (includeStorageFiles && data.files.length > 0) {
            ...
            addStorageFiles(archive, data.files, folderName, storage, exportRef, startTime)
                .then(() => {
                archive.finalize();
            })
                .catch(reject);
 ...
 */
async function addStorageFiles(archive, files, folderName, storage, exportRef, startTime) {
    const bucket = storage.bucket();
    let filesAdded = 0;
    let totalSize = 0;
...
//# sourceMappingURL=processUserExportQueue.js.map