...
exports.analyzeMatchAccuracyCallable = void 0;
const firestore_1 = require("firebase-admin/firestore");
const createCallable_1 = require("../utils/createCallable");
exports.analyzeMatchAccuracyCallable = (0, createCallable_1.createCallable)({ name: "analyzeMatchAccuracy" }, async (ctx, request) => {
    ...
    const rejectedFileMap = new Map(); // txId -> Set<fileId>
    ...
    const dismissedTxMap = new Map(); // fileId -> Set<txId>
    ...
    for (const doc of fileSnap.docs) {
        ...
        // Handle legacy format
        for (const id of (data.dismissedTransactionIds || [])) {
            dismissedIds.add(id);
        }
        // Handle new format
        for (const d of (data.dismissedTransactions || [])) {
    ...
    for (const doc of connectionsSnap.docs) {
        ...
        if (!band && confidence >= 100) {
            ...
            if (lastBand)
                processConnection(lastBand);
        }
        else if (band) {
            processConnection(band);
        }
        function processConnection(b) {
    ...
    for (const doc of connectionsSnap.docs) {
        const conn = doc.data();
        if (conn.connectionType !== "auto_matched")
            continue;
        // Track accuracy by file (partner info would require an extra read)
        const key = conn.fileId;
        if (!partnerAccuracy[key]) {
            partnerAccuracy[key] = { totalMatches: 0, correctCount: 0, accuracy: 0 };
        }
    }
    const autoMatchAccuracy = totalAutoCount > 0 ? Math.round((totalAutoCorrect / totalAutoCount) * 100) : 0;
    const suggestionAcceptRate = totalSuggestionCount > 0
