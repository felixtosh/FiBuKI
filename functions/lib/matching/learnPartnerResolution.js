...
exports.calculateResolutionType = calculateResolutionType;
exports.updatePartnerResolutionStats = updatePartnerResolutionStats;
 ...
 */
function calculateResolutionType(stats) {
    ...
    let confidence = Math.min(THRESHOLDS.MAX_CONFIDENCE, THRESHOLDS.BASE_CONFIDENCE + sampleBoost);
    ...
    const type = fileRatio > noReceiptRatio ? "file_required" : "no_receipt";
 ...
 */
async function updatePartnerResolutionStats(userId, partnerId, resolvedWith, noReceiptCategoryId) {
    const partnerRef = db.collection("partners").doc(partnerId);
    const partnerDoc = await partnerRef.get();
    if (!partnerDoc.exists) {
        console.log(`[learnPartnerResolution] Partner ${partnerId} not found, skipping`);
        return;
    }
    const partner = partnerDoc.data();
    // Verify ownership
    if (partner.userId !== userId) {
        console.log(`[learnPartnerResolution] Partner ${partnerId} not owned by user ${userId}, skipping`);
        return;
    }
    ...
    const newStats = {
        ...
        noReceiptCount: currentStats.noReceiptCount + (resolvedWith === "no_receipt" ? 1 : 0),
    ...
    const newPreference = {
        ...
        preferredNoReceiptCategoryId: resolvedWith === "no_receipt" && noReceiptCategoryId
            ? noReceiptCategoryId
            : partner.resolutionPreference?.preferredNoReceiptCategoryId || null,
        preferredNoReceiptCategoryTemplateId: partner.resolutionPreference?.preferredNoReceiptCategoryTemplateId ||
            null,
    };
    // Get template ID if we have a category ID but no template ID
    if (newPreference.preferredNoReceiptCategoryId &&
        ...
        try {
            ...
            console.error(`[learnPartnerResolution] Failed to fetch category ${newPreference.preferredNoReceiptCategoryId}:`, err);
        }
    }
    // Update partner
    ...
    console.log(`[learnPartnerResolution] Updated partner ${partnerId}: ` +
        `${type} (${confidence}%) - files: ${newStats.fileCount}, noReceipt: ${newStats.noReceiptCount}`);
}
//# sourceMappingURL=learnPartnerResolution.js.map