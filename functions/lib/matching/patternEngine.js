...
exports.formatTxFields = formatTxFields;
...
function buildVerificationPrompt(targetName, proposedPatterns, dryRunResults, totalTransactions, positiveTransactions) {
...
// ============================================================================
function dryRunPatterns(proposedPatterns, allTransactions, ownerId, ownerNameMap) {
    const results = new Map();
    for (const pattern of proposedPatterns) {
        const matches = [];
        for (const tx of allTransactions) {
            if ((0, pattern_utils_1.matchPatternFlexible)(pattern.pattern.toLowerCase(), tx.name || null, tx.partner, tx.reference)) {
 ...
 */
function checkCatastrophicSafety(pattern, matchCount, totalTransactions, sourceTransactionCount) {
 ...
 */
async function learnPatterns(input) {
    const { targetName, positiveTransactions, negativeTransactions, collisionTransactions, allUserTransactions, totalTransactionCount, model, ownerId, ownerNameMap, } = input;
    ...
    const candidatePatterns = aiResult.patterns
        ...
        for (const tx of negativeTransactions) {
            if ((0, pattern_utils_1.matchPatternFlexible)(normalizedPattern, tx.name || null, tx.partner, tx.reference)) {
    ...
    for (const cp of candidatePatterns) {
        ...
        const safety = checkCatastrophicSafety(cp.pattern, matches.length, totalTransactionCount, positiveTransactions.length);
        if (safety.rejected) {
    ...
    const verifyPrompt = buildVerificationPrompt(targetName, safePatterns, dryRunResults, totalTransactionCount, positiveTransactions);
    ...
    if (verifyText) {
        try {
            ...
            verifiedPatterns = safePatterns.filter((cp) => {
                ...
                if (!verification)
                    return true; // Keep if not mentioned
    ...
    if (verifiedPatterns.length > 0 && positiveTransactions.length > 1) {
        const uncoveredTxs = positiveTransactions.filter((tx) => !verifiedPatterns.some((p) => (0, pattern_utils_1.matchPatternFlexible)(p.pattern, tx.name || null, tx.partner, tx.reference)));
        if (uncoveredTxs.length > 0) {
            console.log(`[PatternEngine] Coverage gap: ${uncoveredTxs.length}/${positiveTransactions.length} assigned txs not matched. Retrying...`);
                ...
                if (retryText) {
                    ...
                    if (retryResult.patterns?.length) {
                        const additionalPatterns = retryResult.patterns
                            ...
                            .filter((p) => uncoveredTxs.some((tx) => (0, pattern_utils_1.matchPatternFlexible)(p.pattern, tx.name || null, tx.partner, tx.reference)));
