...
exports.learnScoringWeightsCallable = void 0;
const firestore_1 = require("firebase-admin/firestore");
const createCallable_1 = require("../utils/createCallable");
const db = (0, firestore_1.getFirestore)();
/** Minimum connections with scoreBreakdown needed to compute weights */
const MIN_SAMPLE_SIZE = 5;
exports.learnScoringWeightsCallable = (0, createCallable_1.createCallable)({ name: "learnScoringWeights" }, async (ctx, request) => {
    ...
    for (let i = 0; i < txIdArray.length; i += 30) {
        ...
        for (const doc of connSnap.docs) {
            const conn = doc.data();
            if (!conn.scoreBreakdown)
                continue; // Only connections with breakdown
            const breakdown = conn.scoreBreakdown;
    ...
    if (totalSamples < MIN_SAMPLE_SIZE) {
        console.log(`[ScoringWeights] Not enough data for partner ${partnerId}: ` +
    ...
    console.log(`[ScoringWeights] Partner ${partnerId}: ` +
...
function computeAverages(breakdowns) {
    if (breakdowns.length === 0) {
        return { amount: 0, date: 0, partner: 0 };
    }
    const sum = breakdowns.reduce((acc, b) => ({
 ...
 */
function computeWeight(avgCorrect, avgIncorrect) {
    // If no incorrect data, default to 1.0 (no adjustment needed)
    if (avgIncorrect === 0 && avgCorrect === 0)
        return 1.0;
    // If factor is high in correct but low/zero in incorrect → very discriminating
    if (avgIncorrect === 0)
        return Math.min(1.5, 1.0 + avgCorrect / 40);
    // If factor is high in incorrect but low in correct → misleading
    if (avgCorrect === 0)
        return Math.max(0.5, 1.0 - avgIncorrect / 40);
    // Compare ratio
    const ratio = avgCorrect / avgIncorrect;
    if (ratio > 2.0)
        return 1.3; // Factor strongly discriminates → boost
    if (ratio > 1.5)
        return 1.15; // Factor moderately discriminates → slight boost
    if (ratio < 0.5)
        return 0.7; // Factor is misleading → reduce
    if (ratio < 0.75)
        return 0.85; // Factor is slightly misleading → slight reduce
    return 1.0; // Factor is similar in both → no adjustment
}
//# sourceMappingURL=learnScoringWeights.js.map